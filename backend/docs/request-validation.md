...
Request validation is a critical part of any robust API, ensuring the incoming data meets specified requirements before processing. This documentation covers the request validation system in the NestJS Motorbike System, specifically focusing on the implementation in the `common/request` directory.

The system uses [class-validator](https://github.com/typestack/class-validator) and [class-transformer](https://github.com/typestack/class-transformer) libraries to implement validation, with custom validators extending basic functionality to handle complex business rules.

The documentation explains the features and usage of:
...
3. **Request Pipes**: The `RequestRequiredPipe` in decorator in `src/common/request/pipes/request.required.pipe.ts` ensures that required parameters are present.
4. **RequestDecorators**: The `@RequestTimeout()` decorator in `src/common/request/request.decorator.ts` allows for custom timeout settings
5. **RequestTimeoutInterceptor**: The interceptor in `src/common/request/interceptors/request.timeout.interceptor.ts` handles request timeouts
...
When validation failed, the `RequestValidationException` is thrown:
...
The exception is caught by the `AppValidationFilter` in the `src/app/filters/app.validation.filter.ts`, which formats the error response with detailed validation errors for each invalid property. The response includes
