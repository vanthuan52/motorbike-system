...
import { useVehicleBrandTableColumns } from "../hooks/use-vehicle-brand-table-columns";
...
export default function VehicleBrandList() {
  ...
  const { columns, confirmModalProps } = useVehicleBrandTableColumns({
  ...
  const handleCreate = () => {
    navigate(`${ROUTER_PATH.VEHICLE_BRAND_CREATION}`);
  };
  const statusOptions: SelectOptionItem[] = [
    { value: ENUM_VEHICLE_BRAND_STATUS.ACTIVE, label: "Hoạt động" },
    { value: ENUM_VEHICLE_BRAND_STATUS.INACTIVE, label: "Không hoạt động" },
  ];
  return (
