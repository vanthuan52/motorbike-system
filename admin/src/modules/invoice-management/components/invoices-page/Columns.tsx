...
export const invoiceColumns: ColumnsType<InvoiceManagement> = [
    ...
    },
    {
        ...
        key: "product_type",
        width: 130,
        filters: mockDataTableVehiclePart.map(part => ({
            text: part.name,
            value: part.id,
        })),
        onFilter: (value, record) =>
            record.products.some((p: Product) => p.category_id === value),
        render: (products: Product[]) => {
            const types = Array.from(new Set(products.map((p) => p.category_id)));
            return types.map((type) => {
                const part = mockDataTableVehiclePart.find((p) => p.id === type);
                return part ? <span className="text-base">{part.name}</span> : null;
            });
        },
    },
    {
        ...
        render: (status: string) => (
            <Tag color={paymentStatusColor[status]}>
                <span className="text-base">
                    {status === "completed" ? "Hoàn tất" : status === "pending" ? "Chờ xử lý" : "Thất bại"}
                </span>
            </Tag>
        ),
    },
    ...
    },
    {
        ...
        key: "ship_date",
        width: 140,
        render: (date: string) => <span className="text-base">{dayjs(date).format("HH:mm DD/MM/YYYY")}</span>,
    ...
    },
    {
        ...
        key: "total",
        width: 80,
        render: (total: number) => <span className="text-base">{total.toLocaleString('vi-VN') + ' vnđ'}</span>,
