...
export interface IEmailService {
  ...
  sendWelcomeByAdmin(
    userId: string,
    { email, username }: EmailSendDto,
    { passwordCreatedAt, passwordExpiredAt, password }: EmailCreateByAdminDto,
  ): Promise<void>;
  sendVerification(
    userId: string,
    { email, username }: EmailSendDto,
  ...
  sendWelcome(userId: string, { email, username }: EmailSendDto): Promise<void>;
  sendVerified(
    userId: string,
    { email, username }: EmailSendDto,
  ...
  sendForgotPassword(
    userId: string,
    { email, username }: EmailSendDto,
    { expiredAt, expiredInMinutes, link, reference }: EmailForgotPasswordDto,
