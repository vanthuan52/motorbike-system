...
export default function ScheduleAssignModal({ visible, mode, initialData, onCancel, onSubmit }: Props) {
  ...
  return (
    <Modal
      title={mode === 'edit' ? 'Chỉnh sửa lịch bảo dưỡng' : 'Tạo lịch bảo dưỡng'}
      visible={visible}
      onCancel={onCancel}
      onOk={handleOk}
      destroyOnClose
    >
      <Form form={form} layout="vertical">
        <Form.Item name="customer" label="Khách hàng" rules={[{ required: true, message: 'Nhập tên khách hàng' }]}>           
          <Input placeholder="Nhập tên" />
        </Form.Item>
        <Form.Item name="phone" label="Số điện thoại" rules={[{ required: true, message: 'Nhập số điện thoại' }]}>           
          <Input placeholder="Nhập SĐT" />
        </Form.Item>
        <Form.Item name="schedule_date" label="Ngày bảo dưỡng" rules={[{ required: true, message: 'Chọn ngày bảo dưỡng' }]}>           
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
        <Form.Item name="time_slot" label="Khung giờ" rules={[{ required: true, message: 'Chọn khung giờ' }]}>           
          <Select placeholder="Chọn khung giờ">
            {timeSlots.map(ts => <Select.Option key={ts} value={ts}>{ts}</Select.Option>)}
          </Select>
        </Form.Item>
