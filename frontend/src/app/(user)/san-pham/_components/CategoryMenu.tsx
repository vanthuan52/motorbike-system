...
export default function CategoryMenu({ categories, onCategoryClick, selectedCategories = [] }: Props) {
    return (
        <section>
            <ol className="flex items-center whitespace-nowrap py-4">
                <li className="inline-flex items-center">
                    <CustomLink className="flex items-center text-sm text-gray-500 hover:text-blue-600 focus:outline-hidden focus:text-blue-600" href="/">
                        Trang chủ
                    </CustomLink>
                ...
                {categories.map((cat) => (
                    <SwiperSlide key={cat.id} style={{ width: "150px" }}>
                        ...
                        >
                            ...
                            <p className={`mt-2 text-center text-sm font-medium text-gray-700 ${selectedCategories.includes(cat.id) ? "font-semibold" : ""}`}>{cat.name}</p>
                        </div>
                    </SwiperSlide>
                ))}
            </Swiper>
