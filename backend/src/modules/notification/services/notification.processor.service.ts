...
export class NotificationProcessorService implements INotificationProcessorService {
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping welcome by admin notification',
      };
    }
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping verification email notification',
      };
    }
    ...
    const results = await Promise.allSettled([
      ...
      this.notificationEmailUtil.sendVerificationEmail(emailPayload, data),
    ]);
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping verified mobile number notification',
      };
    }
    ...
    const results = await Promise.allSettled([
      ...
      this.notificationEmailUtil.sendVerifiedMobileNumber(emailPayload, data),
    ]);
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping change password notification',
      };
    }
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping forgot password notification',
      };
    }
  ...
  >): Promise<IQueueResponse> {
    ...
    if (devices.length > 0) {
      ...
      promises.push(this.notificationPushUtil.sendResetPassword(pushPayload));
    }
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!user) {
      return {
        message: 'User not found, skipping new device login notification',
      };
    }
    ...
    const device = this.helperService.resolveDevice(data.requestLog.userAgent);
    const city = this.helperService.resolveCity(data.requestLog.geoLocation);
