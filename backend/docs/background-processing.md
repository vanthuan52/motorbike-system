...
This document explains the background processing system in the NestJS Motorbike System, which uses BullMQ for job queuing and processing, particularly for SMS and email sending.
...
BullMQ is a Redis-based queue system that allows for reliable background job processing in Node.js applications. The boilerplate integrates BullMQ to handle resource-intensive or time-consuming tasks as sendind emails and SMS messages.

### Configuration

The BullMQ configuration is defined in the `CommonModule` through the `BullModule.forRootAsyc()` method. This establishes the Redis connection and default job options for all queues.
...
In the project, the queue registration is primarily done in the route modules rather than in individual service modules. This centralized approach simplifies queue management and ensures consistent configuration.
...
To add SMS jobs to the queue, you need to inject the BullMQ Queue into your service or controller using the `@InjectQueue` decorator from `@nestjs/bullmq`. Note that the SMS queue is registered in the route modules, not the SMS module itself, as explained in the Queue Management Section.
...
- `attempts`: Number of retry attampts (defaults to 3 as configured in the global settings)
...
This architecture allows for a clean seperation between the SMS service functionality and the queue processing logic.
...
SMS templates are defined as text files located in the `/src/modules/sms/templates` diretiory. These are simple text files with placholders that are replaced with actual values when sending an SMS:
...
To add email jobs to the queue, inject the BullMQ Queue into your service or controller using `@InjectQueue` decorator. As described in the Queue Management section, the email queue is registered in the route modules, not in the Email module itself.
...
The project includes several predefined email templates:
...
This approach leverages AWS SES's template capabilities for reliable email delivery with consisten formatting and content.
