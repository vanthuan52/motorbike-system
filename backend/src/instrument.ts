...
import loggerConfigFunction from '@/config/logger.config';
  ...
  Sentry.init({
    ...
    beforeSend(event, hint) {
      if (event.exception?.values) {
        ...
        if (isWorkerException && exception.mechanism?.data?.fatal === false) {
      ...
      if (event.request) {
        ...
        if (helperService.checkUrlMatchesPatterns(url, LoggerExcludedRoutes)) {
          return null;
        }
      }
