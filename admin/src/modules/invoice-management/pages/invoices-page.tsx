...
export default function InvoicesPage() {
  ...
  const filteredData = dataSource.filter((inv) => {
    ...
    const matchPaymentMethod =
      !paymentMethod || inv.payment_method === paymentMethod;
    const matchShippingMethod =
      !shippingMethod || inv.shipping_method === shippingMethod;
    return (
    ...
    <div className="flex flex-col lg:flex-row gap-2 mx-4  my-10 sm:my-2 px-3 py-2 transition-all duration-300 rounded-xl bg-white border border-gray-200 shadow-sm">
      ...
      >
        <InvoiceHeader
          ...
          setPaymentMethod={(v) =>
            handleFilterChange(() => setPaymentMethod(v))
          }
          shippingMethod={shippingMethod ?? ""}
          setShippingMethod={(v) =>
            handleFilterChange(() => setShippingMethod(v))
          }
      ...
      {isMobile ? (
        ...
        <div
          className={`
                    ...
                    `}
          style={{
            minWidth: selectedRow ? 340 : 0,
            maxWidth: selectedRow ? 400 : 0,
          }}
        >
          <InvoiceDetailPanel
            selectedRow={selectedRow || undefined}
