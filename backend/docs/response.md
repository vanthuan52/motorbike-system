...
The Response in NestJs Motorbike System provides a standardized way to structure API response throughout the application. It ensures that all responses follow a consistent format, which improves API usability and makes integration easier for frontend developer.
...
2. **Metadata Enrichment**: Automatically includes metadata like timestamps, timezone, API version, language information, and more.
3. **Internationlization Support**: Integrate with message service to provide localized response messages.
4. **Paigation Support**: Built-in pagination response format for list endpoints, including metadata about the pagination.
5. **File Export**: Support for exporting data in Excel or CSV formats with appropriate headers and context types.
6. **Response Caching**: Optional response cahing capabilities with configurable TTL and custom cache keys.
...
2. **ResponseMetadataDto** (`/dtos/response.dto.ts`)
   ...
   - Contains metadata like `language`, `timestamp`, `timzone`, `path`, `version`

3. **ResponsePagingDto** (`dtos/response.paing.dto.ts`):
...
1. **ResponseInterceptor** (`/interceptors/response.interceptor.ts`):
   ...
   - Transforms basic response to follow the standardized format
...
Simple boolean flag to enable caching with default TTL settings from your Redis configuation:
