...
The pagination is a global module that is automatically imported and configured in the application. It provides a comprehensive set of tools for pagination, filtering, and ordering.

### Services

- **PaginationService**: service that handles page calculation, and order processing.
  - `offset(page, perPage)`: Calculates the offset based on page number and page size
  - `totalPage(totalData, perPage)`: Calcuates the total number of pages
  - `page(page)`: Validates and normalizes the page number (maximun: 20)
  - `perPage(perPage)`: Validates and normalizes the items per page (maximum)
  - `order(orderBy, orderDirection, availableOrderBy)`: Processes ordering parameters
  - `search(searchValue, avaiableSearch)`: Processes search parameters and returns a MongoDB query object

### Decorators

The module provides several decorators to simplify controller implementation:

- **@PaginationQuery(option?)**: Main decorator for handling pagination parameters including search, paging, and ordering
...
- **@PaginationQueryFilterStringContain(field, options?)**: Filter strings containing a value
  ...
  - `field`: Database field to filter
  - `options.queryField`: Custom query parameter name (default to field name)
  - `options.raw`: Return raw value instead of MongoDB query if true

- **PaginationQueryFilterInBoolean(field, defaultValue, options)**: filter by boolean values in an array

  - `field`: Database field to filter
  - `defaultValue`: Default boolean array of not provided
  - `options.queryField`: Custom query parameter name (default to field name)
  - `options.raw`: Return raw value instead of MongoDB query if true
...
- **@PaginationQueryFilterInEnum(field, defaultValue, enumObject, options?)**: Filter by enum values in an array
  ...
  - `options.queryField`: Custom query parameter name (defaults to fieldN name)
  - `options.raw`: Return raw value instead of MongoDB query if true
...
The pagianation pipes transform raw HTTP query parameters into MongoDB query operators through the DatabaseService. Each pipe serves a specific filtering purpose:
...
- **PaginationFilterEqualPipe**: Creates exact math queries using MongoDB's equality operator (`field: value`)
...
- **PaginationFilterDateBetweenPipe**: Generates date rangw queries using MongoDB's `$gte` and `$lte` operators

Each pipe adds its processed parameters to the request instance through the `__pagination` propertym making them available throughou the request lifecycle, and return the approriate MongoDB query object that can merged with other filters.

### DTOs

- **PaginationListDto**: Data transfer object for pagination parameters with the following properties:
  - `_seach`: Search criteria record
  ...
  - `_availbaleOrderDirection`: Array of available order directions
...
The pagination module integrates seamlessly with the database module through the `DatabaseService`. Each pagination filter decorator internally uses the correspoding database helper method:
...
- `@PaginationQueryFilterInBoolean` and `@PaginationQueryFilterInEnum` use `databaseService.filterInt()`
...
The seach functionality in `PaginationService` uses the `DatabaseHelperQueryContain` decorator to create MongoDB `$regex` queries with case-insensitive matching for the specified search fields
