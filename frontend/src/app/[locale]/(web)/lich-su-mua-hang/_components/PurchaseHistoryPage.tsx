...
export default function PurchaseHistoryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
      const filtered = mockInvoices.filter((o) => o.customerId === "cus-01");
      setOrders(filtered);
      setLoading(false);
    }, 800);
  }, []);
  ...
  let filteredOrders = orders.filter((order) => {
    ...
    // Order date filter
    if (orderDateFrom && new Date(order.createdAt) < new Date(orderDateFrom)) {
      return false;
    }
    if (orderDateTo && new Date(order.createdAt) > new Date(orderDateTo)) {
      return false;
    }
    // Ship date filter
    if (shipDateFrom && new Date(order.shipDate) < new Date(shipDateFrom)) {
      return false;
    }
    if (shipDateTo && new Date(order.shipDate) > new Date(shipDateTo)) {
      return false;
    }
    return true;
  });
