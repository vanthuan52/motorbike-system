...
export class LoggerOptionService {
  ...
  constructor(
    ...
    this.autoLogger =
      this.configService.get<boolean>('debug.autoLogger') ?? false;

    this.debugEnable = this.configService.get<boolean>('debug.enable') ?? false;
    this.debugLevel = this.configService.get<string>('debug.level')!;
    this.debugIntoFile =
      this.configService.get<boolean>('debug.intoFile') ?? false;
    this.debugFilePath = this.configService.get<string>('debug.filePath')!;
    this.debugPrettier =
      this.configService.get<boolean>('debug.prettier') ?? false;
  }
        ...
        formatters: {
          log: (object) => {
            ...
            const formatted: { [key: string]: any } = {
              ...
              iso: this.helperDateService.formatToIsoDate(today),
              labels: {
                name: this.name,
                env: this.env,
