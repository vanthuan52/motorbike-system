...
export default function CartSummary({
  ...
  return (
    <div className="bg-surface p-4 sm:p-6 rounded-[var(--radius-xl)] shadow-[var(--shadow-md)] h-full min-h-[500px] flex flex-col relative">
        ...
        {loading
          ...
          : products.map((item) => (
              <div key={item.id} className="flex gap-3 relative">
                <div className="relative w-16 h-16 rounded overflow-hidden bg-surface-alt shrink-0">
                  <Image
                ...
                <div className="absolute -top-2 left-13 z-10">
                  <span className="bg-primary-700 text-white text-[12px] w-5 h-5 flex items-center justify-center rounded-full leading-none">
                    {item.quantity}
                  </span>
                </div>

                <div className="flex-1">
                  <p className="font-medium text-text-primary leading-5 line-clamp-2">
                    {item.name}
                  </p>
      ...
      <div className="pt-4 text-sm space-y-2 flex-1 text-text-secondary">
        {loading ? (
          <div className="space-y-2 animate-pulse">
            <div className="flex justify-between">
              <span className="h-4 bg-secondary-100 rounded w-1/4" />
              <span className="h-4 bg-secondary-100 rounded w-1/6" />
            </div>
            <div className="flex justify-between">
              <span className="h-4 bg-secondary-100 rounded w-1/4" />
              <span className="h-4 bg-secondary-100 rounded w-1/6" />
            ...
            <div className="flex justify-between border-t pt-2 mt-2">
              <span className="h-5 bg-secondary-200 rounded w-1/4" />
              <span className="h-5 bg-secondary-200 rounded w-1/6" />
            </div>
          </div>
        ) : (
          <>
            ...
            <Divider />
            <div className="flex justify-between font-semibold text-base pt-2 text-text-primary">
              <span>{t("total")}</span>
      ...
      <button
        className="bg-primary-700 hover:bg-primary-500 text-white font-medium px-5 py-2 rounded-[var(--radius-md)] cursor-pointer w-full mt-4 sm:mt-0 shadow-[var(--shadow-primary)] hover:shadow-[var(--shadow-primary-hover)] transition-all duration-200"
        onClick={onSubmit}
      >
        {t("proceed")}
      </button>
