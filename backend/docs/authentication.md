...
This documentation covers the authentication system, including JWT authentication, API key authentication, social authentication, and session management
...
JWT authentication in this system uses ES512 (ECDSA using P-521 and SHA-512) algorithm for signing tokens with seperate key pairs for access tokens and refresh tokens.
...
The JWT payload in this system contains the following claims
...
The system includes a script for generating ES512 key pairs:
...
- A JWKS file containing the publi keys for token verification
...
The components are combined for authentication and stored in the database as
...
2. **SYSTEM**: Used for system-level operations, often for service-to-service communication
   - Suitable for: internal services, admin operations, priviledged access
...
This project provides built-in support for social authentication, allowing users to log in using their Google or Apple accounts. Each authentication provider has its own implementation and configuration details
...
- Provides privacy-focues authentication with minimal data collection
...
- Tracks active login session for refresh token only
...
The session management system in this project is designed to work seamlessly with JWT authentication, focusing specifically on refresh token management while allowing access tokens to function independently.

> **Important Not**: Since access tokens have a short lifespan (typically 15-30 minutes) while sessions may be revoked at any time, there is an intentional security gap where a recently revoked sessions's access token might still work until it expires. This is a standard security trade-off that balances security with performance.
