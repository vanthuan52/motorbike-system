...
export default function InvoicesDetailsPage() {
    ...
    const customer = mockDataTableManageCustomers.find(cus => cus.id === invoice?.customer_id);
    if (!invoice) {
        return <div className="text-center text-lg mt-10">Không tìm thấy hóa đơn</div>;
    }
    return (
        <div className="mx-2 sm:mx-4 my-10 sm:my-2 px-2 sm:px-3 py-2 transition-all duration-300 rounded-xl bg-white border border-gray-200 shadow-sm">
            <div className="flex flex-col gap-6">
                <div className="flex flex-row md:items-center md:justify-between gap-2">
                    <div>
                        <div className="flex gap-3 items-center flex-wrap">
                            ...
                            <Tag color={statusColor[invoice.status] || "default"} className="text-base font-medium px-3 py-2">
                ...
                <div className="flex flex-col lg:flex-row gap-4 flex-wrap">
                    ...
                    {customer && <InvoiceCustomerInfo customer={customer} invoice={invoice} />}
                </div>
            </div>
        </div>
    );
