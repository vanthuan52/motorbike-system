...
export class CareRecordAdminController {
  ...
  @CareRecordAdminListCombinedDoc()
  @ResponsePaging('care-record.listByService')
  @PolicyAbilityProtected({
    subject: ENUM_POLICY_SUBJECT.USER,
    action: [ENUM_POLICY_ACTION.READ],
  })
  ...
  async listCareRecordCombined(
    ...
    @Query('vehicleModel', OptionalParseUUIDPipe)
    vehicleModelId: string,
  ): Promise<IResponsePaging<ModelCareRecordListResponseDto>> {
    const find: Record<string, any> = {
      ..._search,
    };
    ...
    const latestPrices: IModelCareRecord[] =
      await this.CareRecordService.getLatestCareRecords();
    const priceMap = new Map(); // Key: `${vehicleServiceId}_${vehicleModelId}`
    latestPrices.forEach((p) =>
      priceMap.set(`${p.vehicleServiceId}_${p.vehicleModelId}`, p),
    );

    const combinedList: IModelCareRecord[] = [];
    allServices.forEach((vehicleService) => {
      allModels.forEach((vehicleModel) => {
        ...
        if (existingPrice) {
          combinedList.push({
            _id: existingPrice.CareRecordId,
          ...
          combinedList.push({
            ...
            dateStart: null,
            dateEnd: null,
            status: ENUM_CARE_RECORD_STATUS.NO_PRICE,
          } as IModelCareRecord);
        }
      });
    });
  ...
  async listByVehicleService(
    ...
    const CareRecords: IModelCareRecord[] =
      await this.CareRecordService.getLatestPricesForService(find, {
        paging: {
          limit: _limit,
          offset: _offset,
        },
    ...
    const total: number =
      await this.CareRecordService.getTotalLatestPricesForService(find);
