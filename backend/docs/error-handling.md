...
The error handling system in NestJs Motorbike System provides a consistent way to handle exceptions throughout the application. It uses NestJS's built-in exception filter mechanism to catch and transform exceptions into standardized HTTP responses.
...
1. **Standardize Error Responses**: All errors follow a consistent structure with approriate HTTP status code, messages, and metadata
2. **Internationalization Support**: Error messages can be localized based on the user's preferred language.
3. **Detailed Validation Errors**: Input validation errors include detailed information about which fields validation and why.
...
The `AppGeneralFilter` is a catch-all filter that handles any unhandled exceptions, including those that don't extend from `HttpException`. It ensures even unexpected errors return a properly formatted response.
...
The `AppHttpFilter` specifically hadnles all HTTP exceptions thrown by the application
...
The `AppValidationFilter` handles validation eceptions that occur during request validation.
...
The error handling syste, is integrated with the application's internationalization system:
...
All error response follow a consistent structure:
