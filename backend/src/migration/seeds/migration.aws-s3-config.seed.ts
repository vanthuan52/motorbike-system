  ...
  implements IMigrationSeed
{
  ...
  async seed(): Promise<void> {
    ...
    if (this.awsS3Service.isMinio()) {
      this.logger.warn(
        'MinIO detected — skipping S3 policy seed. ' +
          'Bucket policies, CORS, and lifecycle rules are configured automatically ' +
          'by the minio-init Docker service at startup.'
      );
      return;
    }
