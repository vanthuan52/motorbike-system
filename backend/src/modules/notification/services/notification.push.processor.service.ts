  ...
  implements INotificationPushProcessorService, OnModuleInit
{
  ...
  >): Promise<IQueueResponse> {
    if (!this.firebaseService.isInitialized()) {
      return {
        message: 'Firebase not initialized, skipping new login notification',
      };
    }
    ...
    if (!notification) {
      return {
        message: 'Notification not found, skipping new login notification',
      };
    }
    ...
    await Promise.allSettled([
      this.notificationPushUtil.sendCleanupTokens(userId, result.failureTokens),
  ...
  >): Promise<IQueueResponse> {
    ...
    if (!notification) {
      return {
        message: 'Notification not found, skipping reset password notification',
      };
    }
