...
export class PaginationService implements IPaginationService {
  ...
  async cursor<TReturn>(
    ...
    const queries: Promise<unknown>[] = [
      repository.findAllCursor(where, {
        cursor: {
    ...
    if (includeCount) {
      queries.push(repository.getTotal(where));
    }
    ...
    if (hasNext) {
      const nextItem = data[data.length - 1]; // Use the last item of the CLEAN data (before slice, but slice only removes the extra one, so last item of 'data' is correct)
      const nextCursorValue = nextItem[cursorField];
  ...
  private decodeCursor(cursor: string): IPaginationCursorValue {
    ...
    if (cursor.length > PaginationMaxCursorLength) {
      throw new BadRequestException({
