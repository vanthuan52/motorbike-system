        ...
        >
            {loading ? (
                ...
                <>
                    ...
                    <div className="text-lg font-semibold mb-2">{selectedRow.id.toUpperCase()}</div>
                    <div className="flex flex-col gap-2 mb-4 text-sm">
                        <div className="flex justify-between py-1">
                            <span className="text-gray-500">Trạng thái thanh toán</span>
                            <Tag color={paymentStatusColor[selectedRow.payment_status]} className="rounded px-3 py-1 text-xs">
                        ...
                        <div className="flex justify-between py-1">
                            ...
                            <Tag color={statusColor[selectedRow.status]} className="rounded px-3 py-1 text-xs">
                        ...
                        <div className="flex justify-between py-1">
                            ...
                            <span className="font-medium">{mockDataTableManageCustomers.find((customer) => customer.id === selectedRow.customer_id)?.first_name + " " + mockDataTableManageCustomers.find((customer) => customer.id === selectedRow.customer_id)?.last_name}</span>
                        </div>
                        <div className="flex justify-between py-1">
                            <span className="text-gray-500">Người nhận</span>
                            <span className="font-medium">{selectedRow.recipient_name ?? ""}</span>
                        </div>
                        <div className="flex justify-between py-1">
                        ...
                        <div className="flex justify-between py-1">
                            ...
                            <span className="text-right max-w-[160px] truncate">{selectedRow.address}</span>
                        </div>
                        <div className="flex justify-between py-1">
                            <span className="text-gray-500">Ngày tạo</span>
                            <span>{dayjs(selectedRow.created_at).format("YYYY-MM-DD HH:mm")}</span>
                        </div>
                        <div className="flex flex-col justify-between py-1">
                            <span className="text-gray-500">Ghi chú</span>
                            {selectedRow.note && (
                                <div className="bg-gray-50 rounded-lg py-2 mt-2 text-gray-600 border border-gray-300">
                                    <span className="p-2 whitespace-pre-line break-words">{selectedRow.note}</span>
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            <div key={p.id} className="flex items-center mb-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="text-xs text-gray-500">
                                        Màu: {p.colors && p.colors.length > 0
                    ...
                    <Button type="default" block onClick={() => window.open(`/invoices/${selectedRow.id}`, "_blank")}>
                        Xem chi tiết
                    </Button>
                </>
            )}
