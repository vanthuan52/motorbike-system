...
import { PartTypePaginationQuery, ENUM_PART_TYPE_STATUS } from "../types";
import { usePartTypeTableColumns } from "../hooks/usePartTypeTableColumns";
...
export default function PartTypeList() {
  ...
  const { columns, confirmModalProps } = usePartTypeTableColumns({
  ...
  const {
    ...
    deletion,
  } = useSelector((state: RootState) => state.partTypes);
  ...
  const handleCreate = () => {
    navigate(`${ROUTER_PATH.PART_TYPE_CREATION}`);
  };
  const statusOptions: SelectOptionItem[] = [
    { value: ENUM_PART_TYPE_STATUS.ACTIVE, label: "Hoạt động" },
    { value: ENUM_PART_TYPE_STATUS.INACTIVE, label: "Không hoạt động" },
  ];
  return (
