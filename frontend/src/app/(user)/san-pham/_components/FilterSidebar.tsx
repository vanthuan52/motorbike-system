...
export default function FilterSidebar({ filters, onChange, categories }: FilterSidebarProps) {
    ...
    const handleCategoryChange = (slug: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
        let next: string[];
        if (e.target.checked) {
    ...
    return (
        <div className="space-y-6 p-2 md:p-0">
            ...
            {/* Danh mục phụ tùng */}
            <div>
                <div className="flex justify-between items-center cursor-pointer select-none" onClick={() => setOpenCategory(v => !v)}>
                    <div className="flex items-center gap-2">
                    ...
                    <span className="cursor-pointer text-gray-400 hover:text-gray-500" onClick={e => { e.stopPropagation(); onChange({ categories: [] }); }}>Reset</span>
                </div>
                <AnimatePresence initial={false}>
                    {openCategory && (
                        <motion.div
