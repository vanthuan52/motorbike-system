...
export default function PartsFormTab({
  ...
  const calcAverage = (vals: any) => {
    const nums = parts.map((part) => Number(vals?.[part.key])).filter((v) => !isNaN(v));
  ...
  return (
      ...
      {showActions && (
        <Form.Item>
          <div className="flex gap-4 justify-center mt-4">
            ...
            <Button onClick={handleSave} className="!text-lg" loading={loadingButton}>
              Lưu
            </Button>
          </div>
        </Form.Item>
    ...
    <div className="flex justify-between items-center">
      {(showBefore || showAfter) && (
        <Form.Item
          label={ <div className='text-left text-lg'>Trung bình trước bảo dưỡng</div>}
     ...
     {showAfter && (
  <Form.Item
    label={ <div className='text-left text-lg'>Trung bình sau bảo dưỡng</div>}
