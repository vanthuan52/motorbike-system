...
The NestJS Motorbike System provides a robust internationalization (i18n) system that enables your application to support multiple languages. The system is built using NestJS's i18n module with a custom implementation that integrates seamlessly with the application's error handling and response mechanisms.

This documentation explains the featues and usage of:

- **Message Module**: Located at `src/common/message`
- **Language Files**: Located at `src/languages`

Internationalization is handled through the `MessageModule` which provides translation services throught the application. Language detection is primarily done through the `x-custom-language` header, and all messages are stored in JSON files within the `src/languages` directory.
...
- `src/languages/en` - English translation
- `src/languages/vi` - Vietnamese translations
...
3. Add the language code to `ENUM_MESSAGE_LANGUGE` enum
