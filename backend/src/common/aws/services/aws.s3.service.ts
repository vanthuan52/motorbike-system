...
export class AwsS3Service implements IAwsS3Service, OnModuleInit {
  ...
  constructor(
    ...
    this.config.set(EnumAwsS3Accessibility.private, {
      ...this.configService.get<IAwsS3ConfigBucket>('aws.s3.config.private'),
  ...
  async checkItem(
    ...
    const item = await this.s3Client.send<
      ...
      HeadObjectCommandOutput
    >(headCommand);
    const { pathWithFilename, extension, mime } = this.getFileInfoFromKey(key);
  ...
  async createMultiPart(
    ...
    if (file.key.startsWith('/')) {
      ...
      throw new Error(`Max part number is greater than ${AwsS3MaxPartNumber}`);
    }
  ...
  async settingBucketPolicy(options?: IAwsS3Options): Promise<void> {
    ...
    if (accessibility === EnumAwsS3Accessibility.public) {
      ...
      const command: DeleteBucketPolicyCommand = new DeleteBucketPolicyCommand({
        Bucket: config.bucket,
      });
