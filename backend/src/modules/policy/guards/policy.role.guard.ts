...
import { POLICY_ROLE_META_KEY } from '../constants/policy.constant';
...
export class PolicyRoleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const { __user, user } = context.switchToHttp().getRequest<IRequestApp>();
    if (!__user || !user) {
    ...
    if (type === ENUM_POLICY_ROLE_TYPE.SUPER_ADMIN) {
      ...
      throw new ForbiddenException({
        statusCode: ENUM_POLICY_STATUS_CODE_ERROR.ROLE_PREDEFINED_NOT_FOUND,
